...
class Extract:
    ...
    def load_metadata_file(self) -> None:
        main_logger.info("Metadata filepath is %s.", self.config.get_metadata_filepath())
        ...
        main_logger.debug(filename)
        main_logger.debug(self.metadata[MetadataColumns.DATASET_NAME.value].unique())
        if filename not in self.metadata[MetadataColumns.DATASET_NAME.value].unique():
            main_logger.error("The current dataset is not described in the provided metadata file.")
            exit()
        else:
        ...
        main_logger.info("%s columns and %s lines in the metadata file.", len(self.metadata.columns), len(self.metadata))
    ...
    def preprocess_metadata_file(self) -> None:
        ...
        main_logger.debug("working on hospital %s", self.config.get_hospital_name())
        ...
        main_logger.debug(self.metadata.columns)
        main_logger.debug(columns_to_keep)
        ...
        main_logger.debug("will drop %s in %s", self.config.get_hospital_name(), self.metadata.columns)
        self.metadata = self.metadata.drop(self.config.get_hospital_name(), axis=1)
        main_logger.debug(self.metadata)

    def load_data_file(self) -> None:
        main_logger.info(self.config.get_current_filepath())
        ...
        main_logger.info("Data filepath is %s.", self.config.get_current_filepath())
        ...
        main_logger.info("%s columns and %s lines in the data file.", len(self.data.columns), len(self.data))
    ...
    def compute_mapped_values(self) -> None:
        ...
        main_logger.debug(self.mapped_values)
    ...
    def compute_mapped_types(self) -> None:
        ...
        main_logger.debug(self.mapped_types)
    ...
    def run_value_analysis(self) -> None:
        ...
        for column in self.data.columns:
            ...
            # main_logger.debug("Values are: %s", values)
            # main_logger.debug(self.metadata["name"])
            # main_logger.info("Working on column '%s'", column)
            ...
            if value_analysis.nb_unrecognized_data_types > 0 or (0 < value_analysis.ratio_non_empty_values_matching_accepted < 1):
                main_logger.info("%s: %s", column, value_analysis)
    ...
    def run_variable_analysis(self) -> None:
        ...
        main_logger.info(variable_analysis)
