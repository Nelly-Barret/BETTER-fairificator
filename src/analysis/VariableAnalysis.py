...
class VariableAnalysis:
    ...
    def compute_nb_variables_without_ontology(self):
        ...
        print(total_number_variables_with_data)
        main_logger.info("total_number_variables_with_data = %s", total_number_variables_with_data)
        self.ratio_variable_no_ontology = nb_variables_without_ontology / total_number_variables_with_data
        main_logger.debug("Number of variables without ontology: %s/%s=%s", nb_variables_without_ontology, total_number_variables_with_data, self.ratio_variable_no_ontology)
    ...
    def compute_nb_categorical_features_without_mapping(self):
        ...
        for index, metadata_variable in self.metadata.iterrows():
            if metadata_variable[MetadataColumns.VAR_TYPE.value] == "category":
                if not is_not_nan(metadata_variable[MetadataColumns.JSON_VALUES.value]):
                    main_logger.debug(metadata_variable[MetadataColumns.COLUMN_NAME.value])
        ...
        main_logger.debug("Ratio of categorical feature having no mapping: %s/%s=%s", self.nb_categorical_features_without_mapping, self.total_nb_categorical_features, self.ratio_categorical_feature_with_no_mapping)
